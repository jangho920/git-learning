...
<html lang="en">
    <head>
        ...
        <title>Dealer Modification</title>
        ...
        <script type="text/javascript">
            ...
            var fnSelectDealerInfo = function(){
            	var param = {
                        "url"    : "/dealerInfo/selectDealerInfo.ajax"
            ...
            var succCallback = function(param){
            	dealerInfo = param;
            	$("#dealerId").val(dealerInfo.dealerId);
            	$("#dealerNm").val(dealerInfo.dealerNm);
            	$("#repNm").val(dealerInfo.repNm);
            	$("#email").val(dealerInfo.email); 
            	if(!StringUtil.isNull(dealerInfo.telNum) && dealerInfo.telNum.split("-").length == 3){
            		$("#telNum1").val(dealerInfo.telNum.split("-")[0]); 
                	$("#telNum2").val(dealerInfo.telNum.split("-")[1]); 
                	$("#telNum3").val(dealerInfo.telNum.split("-")[2]); 	
            	}
            	$("#zipCd").val(dealerInfo.zipCd); 
            	$("#addr1").val(dealerInfo.addr1);
            	$("#addr2").val(dealerInfo.addr2);
            }
            ...
            $(document).on("click", "#btnSave", function(e){
        		...
        		var param = {
                        "url"          : "updateDealerInfo.ajax"	
                      , "data"         : {  "dealerId"      : '<c:out value="${data.dealerId}" />'
    	                                  , "dealerNm"       : $("#dealerNm").val()
    	                                  , "repNm"         : $("#repNm").val()
    	                                  , "email"         : $("#email").val()
    ...
    <body>   
        <div class="bbs-cond flex-md mb15">
            <h1 class="ti mb0">Dealer Modification</h1>
            <div class="cond-btns">
        ...
        <div class="cardCont">
            <div class="formbox pt0">
                <label for="dealerNm" class="req">Dealer Name</label>
                <div class="form-inp"><input type="text" class="inp" id="dealerNm" maxlength="30"></div>
            </div>
